...
class ShippingPricingManager
{
    ...
    public function find_valid_range($package, $total_weight, $total_value, $pricing_data, $unit_converter, $cart_analyzer)
    {
        ...
        if (! $this->all_products_match_method_categories($package, $pricing_data, $cart_analyzer)) {
            return ValidationResult::hide();
        }
    ...
    private function all_products_match_method_categories($package, $pricing_data, $cart_analyzer)
    {
        // Collect all unique categories from all ranges
        $all_categories = [];
        foreach ($pricing_data->ranges as $range) {
            if (is_object($range) && isset($range->categories) && ! empty($range->categories)) {
                $all_categories = array_merge($all_categories, $range->categories);
            }
        }
        ...
        // If no categories specified, all products match
        ...
        foreach ($package['contents'] as $item) {
            $product = $item['data'];
            $product_has_matching_category = false;

            foreach ($all_categories as $category_slug) {
                if ($this->product_in_category($product, $category_slug)) {
                    $product_has_matching_category = true;
                    break;
                }
            }

            // If this product doesn't have any of the method's categories, hide method
            if (! $product_has_matching_category) {
                return false;
            }
        }
